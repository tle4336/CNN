...
class CONV_LAYER(object):
    ...
    def __init__(self, layer_size, kernel_size, fan, **params):
        ...
        if fname:
            ...
            #Generate random weight initialization
            ...
            #Generate zero weight initialization
            #self.kernel = np.zeros(fan[1], kernel_size)
            #self.bias = np.zeros(fan[1], kernel_size[0])
            
            #Generate He-initialization
            #self.kernel = np.random.randn(fan[1], fan[0], kernel_size)* (np.sqrt(2. / fan[0]))
            #self.bias = np.zeros(fan[1], kernel_size[0])
            
            #Generate Xavier-initialization
            #self.kernel = np.random.randn(fan[1], fan[0], kernel_size)* (np.sqrt(1. / fan[0]))
            #self.bias = np.zeros(fan[1], kernel_size[0])            
